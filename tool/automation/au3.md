...
### 使用原生函数定位
> * 1、打开AutoIt Window Info，Options中取消Freeze
> * 2、鼠标移动到对应的图标上，查看相关信息
> * 3、示例： [pyautoit示例](https://pypi.org/project/PyAutoIt/)

### 使用 [Accessibility Insights for Windows](https://accessibilityinsights.io/docs/en/windows/overview/) 分析 应用 与 元素
> * 1、打开软件, 将鼠标移动至对应的鼠标上即可获取到 对象（app/element） 的属性
> * 2、控件的属性含义，参考 [官方属性标识符说明](./Windows_accessible_API_reference.md#1-5-1-6-1)
> * 3、事件的监听：选中对象，右键，选择监听事件。
> * 4、定位当前元素
>   * 4-1、使用_UIA_getFirstObjectOfElement获取 所有桌面元素$UIA_oDesktop 的 孩子$treescope_children中 符合条件（通过class名称筛选）的元素，得到对象obj
>   * 4-2、调用obj.setfocus定位元素，使用_UIA_DumpThemAll将obj的所有$treescope_subtree落地为xml文件
